...
export class CreateProfileComponent {
  ...
  createAccount() {
    if (!this.bioText || !this.username || !this.selectedChampionName || !this.iconID) {
      console.error('Missing required fields');
      return;
    }
    this.mono
      .postCreateProfile({
        nick: this.username.toLocaleLowerCase(),
        bio: this.bioText,
        champion: this.selectedChampionName,
        iconID: this.iconID,
        karma: 0,
        elo: 'Mestre',
        pdl: 1,
  ...
  toggleIconSelection(iconIndex: number): void {
    ...
    if (selectedIcon && !selectedIcon.classList.contains('selected')) {
      ...
      this.iconID = Number(championIcons![iconIndex]);
      this.selectedIconURL = `${
        this.iconBaseURL + championIcons![iconIndex]
      }.webp`;
    }
  }
}
